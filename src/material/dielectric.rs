...
use crate::{color::Color, ray::Ray, util::random_double, vec3::Vec3};
...
impl Material for Dielectric {
    fn scatter(
        ...
        let cos_theta = -unit_direction.dot_product(hit_record.normal).min(1.0);
        let sin_theta = (1.0 - cos_theta.powi(2)).sqrt();

        let cannot_refract = refraction_ratio * sin_theta > 1.0;
    
        let direction = if cannot_refract || schlick_reflectance(refraction_ratio, cos_theta) > random_double() {
            Metal::reflect(unit_direction, hit_record.normal)
        } else {
            Dielectric::refract(unit_direction, hit_record.normal, refraction_ratio)
        };
...
impl Dielectric {
    ...
    fn refract(ray: Vec3, normal: Vec3, refraction_index: f64) -> Vec3 {
        ...
        let cos_theta = f64::min(-ray.dot_product(normal), 1.0);
